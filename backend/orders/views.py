    ...
    def create(self, request, *args, **kwargs):
        ...
        for item in items_data:
            ...
            oi = OrderItem.objects.create(
                ...
                unit_price=unit_price, unit_cost=float(product.purchases.last().unit_cost) if product.purchases.exists() else 0
            )
...
class GenerateMercadoPagoView(APIView):
    def post(self, request, pk):
        ...
        preference_data = {
            ...
            "back_urls": {"success": f"{settings.WHATSAPP_SERVICE_URL}/mp/success", "failure": f"{settings.WHATSAPP_SERVICE_URL}/mp/failure"},
...
class ExportOrdersView(APIView):
    def get(self, request):
        ...
        for o in Order.objects.all().order_by('-created_at'):
            ws.append([o.id, o.customer.get_full_name(), float(o.total), o.status, o.delivery_type, o.delivery_address, o.payment_method, o.payment_status, str(o.created_at.date())])
...
class DashboardView(APIView):
    def get(self, request):
        ...
        return Response({
            ...
            'accounts_receivable': float(Order.objects.filter(payment_status='pendiente', payment_condition='plazo').aggregate(t=Sum('total'))['t'] or 0),
