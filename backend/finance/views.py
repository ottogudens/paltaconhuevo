...
class FinanceSummaryView(APIView):
    def get(self, request):
        ...
        elif period == 'week': start = today - datetime.timedelta(days=7)
        ...
        return Response({'ingresos': ingresos, 'egresos': egresos, 'balance': ingresos - egresos, 'period': period})
...
class ExportTransactionsView(APIView):
    def get(self, request):
        ...
        for t in Transaction.objects.all().order_by('-date'):
            ws.append([t.id, t.transaction_type, t.category, float(t.amount), t.description, t.reference_id, str(t.date)])
